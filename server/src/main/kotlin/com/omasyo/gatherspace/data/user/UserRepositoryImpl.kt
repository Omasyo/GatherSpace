...
class UserRepositoryImpl(
    private val userAccountQueries: User_accountQueries, private val refreshTokenQueries: Refresh_tokenQueries
) : UserRepository {
    override fun create(username: String, password: String, imageBuffer: Buffer?): DatabaseResponse<Unit> = try {
    ...
    override fun update(id: Int, username: String?, imageBuffer: Buffer?): DatabaseResponse<Unit> = try {
        userAccountQueries.transactionWithResult {
            val user = userAccountQueries.getById(id).executeAsOneOrNull()
                ?: return@transactionWithResult DatabaseResponse.Failure(
                    "This user does not exists", HttpStatusCode.NotFound.value
                )
            ...
            DatabaseResponse.Success(
                userAccountQueries.update(
                    username = username, image = imageId, id = id
                )
            )
        }
    } catch (e: PSQLException) {
    ...
    override fun updatePassword(id: Int, password: String): DatabaseResponse<Unit> {
        return try {
            userAccountQueries.transactionWithResult {
                DatabaseResponse.Success(
                    userAccountQueries.updatePassword(
    ...
    override fun getUserById(id: Int): UserDetails? = userAccountQueries.getById(id, ::userMapper).executeAsOneOrNull()


    override fun getUserByUsername(username: String): UserDetails? =
        userAccountQueries.getByUsername(username, ::userMapper).executeAsOneOrNull()


    override fun validateUser(username: String, password: String): Boolean = userAccountQueries.validateUser(
        username = username, password = password
    ).executeAsOneOrNull() ?: false


    override fun getUserSessions(userId: Int): List<UserSession> =
        refreshTokenQueries.getByUserId(userId) { _, deviceId, deviceName, created, lastAccessed ->
            UserSession(
                userId = userId,
                deviceId = deviceId,
...
fun userMapper(
    userId: Int, username: String, image: String?, created: LocalDateTime, modified: LocalDateTime
) = UserDetails(
    id = userId,
    username = username,
